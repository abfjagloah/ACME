...
class ResGCN(torch.nn.Module):
    ...
    def __init__(self, dataset, hidden, num_feat_layers=1, num_conv_layers=3,
        ...
        assert "sum" in global_pool or "mean" in global_pool or "max" in global_pool, global_pool
    ...
    def forward(self, data):
        ...
        if self.collapse:
            return self.forward_collapse(x, edge_index, edge_weight, batch, xg)
        elif self.res_branch == "BNConvReLU":
            return self.forward_BNConvReLU(x, edge_index, edge_weight, batch, xg)
        elif self.res_branch == "BNReLUConv":
            return self.forward_BNReLUConv(x, edge_index, edge_weight, batch, xg)
        elif self.res_branch == "ConvReLUBN":
            return self.forward_ConvReLUBN(x, edge_index, edge_weight, batch, xg)
        elif self.res_branch == "resnet":
            return self.forward_resnet(x, edge_index, edge_weight, batch, xg)
        else:
            raise ValueError("Unknown res_branch %s" % self.res_branch)

    def forward_collapse(self, x, edge_index, edge_weight, batch, xg=None):
        x = self.bn_feat(x)
    ...
    ### This one!!!
    def forward_BNConvReLU(self, x, edge_index, edge_weight, batch, xg=None):
        x = self.bn_feat(x)
    ...
    def forward_cl(self, data):
        ...
        return self.forward_BNConvReLU_cl(x, edge_index, edge_weight, batch, xg)

    def forward_BNConvReLU_cl(self, x, edge_index, edge_weight, batch, xg=None):
        x = self.bn_feat(x)
        x = F.relu(self.conv_feat(x, edge_index, edge_weight))
        for i, conv in enumerate(self.convs):
            x_ = self.bns_conv[i](x)
    ...
    def forward_BNReLUConv(self, x, edge_index, edge_weight, batch, xg=None):
        x = self.bn_feat(x)
        x = self.conv_feat(x, edge_index, edge_weight)
        for i, conv in enumerate(self.convs):
            x_ = F.relu(self.bns_conv[i](x))
            x_ = conv(x_, edge_index, edge_weight)
            x = x + x_ if self.conv_residual else x_
    ...
    def forward_ConvReLUBN(self, x, edge_index, edge_weight, batch, xg=None):
        x = self.bn_feat(x)
        x = F.relu(self.conv_feat(x, edge_index, edge_weight))
        x = self.bn_hidden(x)
        for i, conv in enumerate(self.convs):
            x_ = F.relu(conv(x, edge_index, edge_weight))
