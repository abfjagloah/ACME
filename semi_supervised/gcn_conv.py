...
class GCNConv(MessagePassing):
    ...
    @staticmethod
    def norm(edge_index, num_nodes, edge_attr, improved=False, dtype=None):
        ...
        # Add edge_attr for loop edges.
        loop_attr = torch.full((num_nodes, ),
        ...
        row, col = edge_index
        deg = scatter_add(edge_attr, row, dim=0, dim_size=num_nodes)
        deg_inv_sqrt = deg.pow(-0.5)
        deg_inv_sqrt[deg_inv_sqrt == float('inf')] = 0

        return edge_index, deg_inv_sqrt[row] * edge_attr * deg_inv_sqrt[col]
        ...
        if not self.cached or self.cached_result is None:
            if self.edge_norm:
                edge_index, norm = GCNConv.norm(
                    edge_index, x.size(0), edge_attr, self.improved, x.dtype)
            else:
                norm = None
